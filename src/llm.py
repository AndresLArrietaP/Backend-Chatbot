...
def _reforzar_join_proyecto_modelo_aceite(q: str) -> str:
    return (
        ...
        + " | IMPORTANTE — CADENA DE JOIN para análisis de aceite con proyecto y/o modelo: "
          ...
          "SIEMPRE agrega WHERE LD.[Compartimiento] IS NOT NULL para excluir filas sin componente. "
          "SIEMPRE agrega AND LD.[FechaMuestreo] >= DATEADD(YEAR, -5, GETDATE()) para limitar el scan. "
          "Si la consulta pide la ÚLTIMA muestra POR componente/compartimiento, "
          "usa el patrón MAX+GROUP BY (más eficiente que ROW_NUMBER en esta BD): "
          "CTE_Maximas: GROUP BY LD.[MiningEquipmentId], LD.[Compartimiento] con MAX(LD.[FechaMuestreo]) AS UltimaFecha. "
          "Luego JOIN de vuelta a [Oil].[LaboratoryData] ON MiningEquipmentId, Compartimiento y FechaMuestreo = UltimaFecha. "
          "Aplica TOP 200 en el SELECT final (no dentro del CTE). "
          "Si pide las últimas N muestras (N>1) por componente, entonces sí usa "
          "ROW_NUMBER() OVER (PARTITION BY LD.[Compartimiento] ORDER BY LD.[FechaMuestreo] DESC) AS rn "
          "y filtra WHERE rn <= N."
    )
