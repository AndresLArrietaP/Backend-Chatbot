...
def _construir_prompt_retry_cero_filas_semantico(
    ...
    return (
        ...
        f"usa ROW_NUMBER() OVER (PARTITION BY [MiningEquipmentId],[Compartimiento] ORDER BY [FechaMuestreo] DESC) AS rn "
        f"y filtra rn=1 en el SELECT externo (NUNCA GROUP BY+MAX — la BD tiene duplicados por fecha que generan datos incorrectos).\n"
