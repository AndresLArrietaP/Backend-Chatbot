...
""".strip()
    ...
    async def construir_respuesta(
        ...
        prompt = f"""
Eres un ANALISTA SENIOR de datos especializado en resultados tabulares y análisis operativo.
...
OBLIGATORIO:
- Redacta como un analista senior: primero una síntesis ejecutiva y luego hallazgos puntuales.
- Si hay métricas numéricas, menciona máximo, mínimo, mediana y p90 cuando aporten valor.
...
- Si la evidencia es insuficiente para inferir tendencia, dilo explícitamente.
- No dejes frases cortadas.
...
""".strip()
        ...
        if resumen_deterministico:
            return f"{resumen_deterministico}\n{respuesta_local}".strip()
