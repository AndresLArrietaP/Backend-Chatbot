...
# JOIN + NULL-SAFE AGGREGATION (VERY IMPORTANT):
# - Default JOIN strategy:
#   - Use INNER JOIN only when the relationship is mandatory OR the user explicitly asks for "only matched records".
#   - Use LEFT JOIN when the relationship may be optional/nullable to avoid dropping rows.
...
#   - If strict results requested (exclude null / only matches):
...
#   - Choose fallbacks from available TEXT columns in the involved tables (do not invent columns).
#     Prefer descriptive labels such as: Name, Description, Code, Type, Category, Model, System, Project, Customer, etc.
#   - Consistency rule:
...
#         def _usuario_pide_estricto(self, consulta: str) -> bool:
#             q = (consulta or "").lower()
#             pistas = [
...
#             # Null-safe GROUP BY (genérico) para MSSQL: evita grupos NULL por LEFT JOIN
#             if (dialecto or "").lower() in ("mssql", "sqlserver"):
#                 sql2 = self._hacer_groupby_nullsafe_mssql(sql2, consulta=consulta, esquema_json=esquema_json)
...
Módulo: providers.gemini_client
--------------------------------
Cliente Gemini (Google GenAI) para:
- NL → SQL (JSON robusto) con timeouts, hedging y fallback.
- (Opcional) construir respuesta en lenguaje natural basada en filas/resultados.

FIX PRINCIPAL (MSSQL):
- Evitar GROUP BY de campos provenientes de tablas LEFT JOIN que terminan agrupando todo en NULL.
- Regla:
  - Si el usuario NO pide “estricto”, se agrupa por COALESCE(dim, fallback1, fallback2)
  - Si el usuario pide “estricto” (solo matches), se filtra WHERE dim IS NOT NULL
- Los fallbacks se eligen sin inventar:
  - Solo columnas ya referenciadas en el SQL
  - Solo columnas tipo texto (según schema_json)
"""
...
class GeminiProvider:
    ...
    Proveedor Gemini (Google GenAI).
    Implementa:
    - consulta_humana_a_sql (principal)
    - ping
    - listar_modelos
    """
    ...
    # =======================
    #  NULL-SAFE GROUP BY MSSQL (GENÉRICO)
    # =======================
    ...
    def _usuario_pide_estricto(self, consulta: str) -> bool:
        ...
        pistas = [
            "solo coincidencias", "solo matching", "sin null", "no null", "excluir null",
            "solo donde exista", "solo donde haya", "únicamente los que tengan",
            "estricto", "exactamente", "obligatorio", "solo los que tengan"
        ]
        return any(p in q for p in pistas)
    ...
    def _extraer_left_join_aliases(self, sql: str) -> List[str]:
        aliases: List[str] = []
        for m in re.finditer(
            r"\bleft\s+join\s+(?P<t>(?:\[[^\]]+\]|\w+)(?:\s*\.\s*(?:\[[^\]]+\]|\w+))?)\s+(?:as\s+)?(?P<a>\[[^\]]+\]|\w+)\b",
            sql or "",
            flags=re.IGNORECASE
        ):
            aliases.append(m.group("a") or "")
    ...
    def _es_tipo_texto(self, tipo: str) -> bool:
        t = (tipo or "").lower()
        return any(x in t for x in ["char", "text", "varchar", "nvarchar", "string", "clob"])
    ...
    def _es_texto_por_schema(self, esquema_json: Dict[str, Any], table_name: str, col_name: str) -> bool:
        ...
        for tb in (esquema_json.get("tables") or []):
            ...
            for col in (tb.get("columns") or []):
                if (col.get("name") or "").strip().lower() == c:
                    return self._es_tipo_texto(col.get("type") or "")
        return False

    def _elegir_fallbacks(self, sql: str, esquema_json: Dict[str, Any], alias_map: Dict[str, str], excluye_alias: str, excluye_col: str) -> List[str]:
        """
        Fallbacks = columnas TEXT ya referenciadas en el SQL (alias.col) que NO sean dim.
        No inventa columnas.
        """
        refs: List[str] = []
        for m in re.finditer(
            ...
            a = a_raw.replace("[", "").replace("]", "").strip().lower()
            c = c_raw.replace("[", "").replace("]", "").strip()

            if not a or not c:
                continue
            if a == excluye_alias.lower() and c.lower() == excluye_col.lower():
                continue
    ...
    def _pat_dim_variants(self, alias_norm: str, col_norm: str) -> str:
        """
        Regex que matchea:
          c.[Col] | [c].[Col] | c.Col | [c].Col
        """
        a = re.escape(alias_norm)
        c = re.escape(col_norm)
        return (
            rf"(?:\[\s*{a}\s*\]|\b{a}\b)\s*\.\s*"
            rf"(?:\[\s*{c}\s*\]|\b{c}\b)"
        )
    ...
    def _hacer_groupby_nullsafe_mssql(self, sql: str, consulta: str, esquema_json: Dict[str, Any]) -> str:
        ...
        - Detecta LEFT JOIN aliases
        - Si GROUP BY usa alias.col donde alias ∈ LEFT JOIN aliases
        - Estricto -> WHERE dim IS NOT NULL
        - Default -> COALESCE(dim, fallback1, fallback2) (y GROUP BY mismo expr)
        """
        s = (sql or "").strip()
        if not s or "group by" not in s.lower():
            return s
        ...
        left_norm = {a.replace("[", "").replace("]", "").strip().lower() for a in left_aliases}
        alias_map = self._alias_to_table(s, esquema_json)
        ...
        group_list = mg.group("g").strip()
        exprs = [e.strip() for e in group_list.split(",") if e.strip()]
        ...
        a, c = target
        dim = f"[{a}].[{c}]"
        dim_pat = self._pat_dim_variants(a, c)

        if self._usuario_pide_estricto(consulta):
            if re.search(r"\bwhere\b", s, flags=re.IGNORECASE):
                s = re.sub(r"\bwhere\b", f"WHERE {dim} IS NOT NULL AND", s, count=1, flags=re.IGNORECASE)
            else:
                s = re.sub(r"\bgroup\s+by\b", f"WHERE {dim} IS NOT NULL GROUP BY", s, count=1, flags=re.IGNORECASE)
        ...
        fallbacks = self._elegir_fallbacks(s, esquema_json=esquema_json, alias_map=alias_map, excluye_alias=a, excluye_col=c)
        if not fallbacks:
            # al menos evita el grupo NULL
            if re.search(r"\bwhere\b", s, flags=re.IGNORECASE):
                s = re.sub(r"\bwhere\b", f"WHERE {dim} IS NOT NULL AND", s, count=1, flags=re.IGNORECASE)
            else:
                s = re.sub(r"\bgroup\s+by\b", f"WHERE {dim} IS NOT NULL GROUP BY", s, count=1, flags=re.IGNORECASE)
        ...
        # 1) GROUP BY: reemplazar primera ocurrencia de la dimensión
        group_new = re.sub(dim_pat, expr_coalesce, group_list, count=1, flags=re.IGNORECASE)
        s = s[:mg.start("g")] + group_new + s[mg.end("g"):]

        # 2) SELECT: si aparece la dimensión “suelta”, reemplazarla y dejar alias = nombre de columna
        ms = re.search(r"^\s*select\s+(?P<body>.+?)\bfrom\b", s, flags=re.IGNORECASE | re.DOTALL)
        if ms:
            body = ms.group("body")
            body_new = re.sub(
                dim_pat,
                f"{expr_coalesce} AS [{c}]",
                body,
                count=1,
                flags=re.IGNORECASE
            )
    ...
    def _posprocesar_sql(self, sql: str, consulta: str, esquema_json: Dict[str, Any], dialecto: str) -> str:
        sql2 = (sql or "").strip()
        if (dialecto or "").lower() in ("mssql", "sqlserver"):
            sql2 = self._hacer_groupby_nullsafe_mssql(sql2, consulta=consulta, esquema_json=esquema_json)
        return sql2
    ...
    async def consulta_humana_a_sql(
        ...
        """
        Convierte una consulta en lenguaje natural a SQL (solo lectura), devolviendo JSON:
          { "sql_query": "...", "original_query": "..." }
        """
        esquema_compacto = [
            {
        ...
        fq_names = [t.get("fq_name") for t in (esquema_json.get("tables", []) or []) if t.get("fq_name")]

        dd = (dialecto or "").lower().strip()
        if dd in ("mssql", "sqlserver"):
            dialect_rules = f"""
DIALECT: SQL SERVER (T-SQL / Azure SQL)
- Use TOP ({limite_por_defecto}) (no LIMIT).
- Use square brackets: [dbo].[Table], [Column].
- Prefer fully qualified names EXACTLY as in <allowed_fqn>.
- LEFT JOIN when relationship may be optional.
- If LEFT JOIN + GROUP BY a right-table field:
  - Strict user -> WHERE dim IS NOT NULL, GROUP BY dim
  - Default -> GROUP BY COALESCE(dim, fallback1, fallback2)
"""
        else:
            dialect_rules = f"DIALECT: POSTGRESQL - Use LIMIT {limite_por_defecto}"

        system_instruction = f"""
You are a careful SQL generator for {dialecto.upper()}.
Hard rules:
- Output READ-ONLY: SELECT/CTE/EXPLAIN (without ANALYZE). No INSERT/UPDATE/DELETE/DDL/CALL.
- Use only tables/columns from the provided schema. Do not hallucinate.
- Prefer fully-qualified names exactly as in <allowed_fqn>.
- Always include a row limit in the dialect-appropriate way.
...
""".strip()
        ...
        # ✅ Post-proceso real, siempre aplicado
        sql = self._posprocesar_sql(sql, consulta=consulta, esquema_json=esquema_json, dialecto=dialecto)

        obj["sql_query"] = sql
        obj.setdefault("original_query", consulta)
        return json.dumps(obj, ensure_ascii=False)

    # --------------------------------- Ping -----------------------------------

    async def ping(self, model: Optional[str] = None, modelo: Optional[str] = None) -> str:
        """Compat: soporta ping(model=...) y ping(modelo=...)"""
        msel = model or modelo
        primario_full = self._resolver_modelo(msel)
        modelos = self._cadena_fallback(primario_full, "GEMINI_MODEL_CANDIDATES")
        ultimo = ""
        for m in modelos:
            try:
    ...
    # Alias legacy (por si algún módulo viejo los usa)
    async def human_query_to_sql(self, human_query: str, schema_json: Dict[str, Any], dialect: str = "postgresql", default_limit: int = 100, model: Optional[str] = None) -> str:
