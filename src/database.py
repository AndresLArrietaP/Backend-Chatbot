...
Módulo: database
----------------
Capa de acceso a datos (SQLAlchemy) con guardrails de seguridad para ejecutar
consultas SQL generadas por LLM de forma controlada.

Responsabilidades:
  1. Engine / sesión     — Crea el engine SQLAlchemy (mssql+pymssql o postgresql).
  2. Introspección       — obtener_esquema_json(): tablas+columnas con caché en RAM.
  3. Limpieza de SQL     — limpiar_sql(): quita markdown fences, comentarios, punto y coma.
  4. Macros              — expandir_macros(): NUMERIC_CLEAN / DATE_PARSE → SQL puro Postgres.
                          En MSSQL se desactivan automáticamente.
  5. Seguridad           — es_select_seguro(): solo SELECT/CTE/EXPLAIN/VALUES,
                          sin DROP/DELETE/INSERT/UPDATE/ALTER/TRUNCATE.
  6. Allow-list          — restringir_a_tablas_permitidas(): verifica que el SQL solo use
                          tablas del esquema introspectado.
  7. Calificación        — calificar_tablas(): FROM Tabla → FROM [schema].[Tabla].
  8. Límite de filas     — forzar_limit(): TOP(N) en MSSQL, LIMIT N en Postgres.
  9. Reescritura NULL    — hacer_groupby_nullsafe() / hacer_coalesce_proyeccion_nula():
                          usados en reintentos automáticos desde main.py.
  10. Ejecución          — consultar(): ejecuta el SQL validado → lista de dicts.

Dialectos soportados:
  SQL Server / Azure SQL  → TOP(N), corchetes, sin LIMIT, sin sslmode.
  PostgreSQL              → LIMIT, comillas dobles, sslmode=require.

Aliases legacy (compatibilidad):
  get_schema_json, clean_sql, expand_macros, query, qualify_tables,
  invalidate_schema_cache, refrescar_cache_esquema.

Configuración (.env):
  DATABASE_URL, DB_DIALECT, ALLOW_SQL_EXPLAIN, ALLOW_EXPLAIN_ANALYZE,
  ALLOW_SQL_VALUES, ALLOW_SQL_CALL, SQL_AUTO_LEFT_JOIN_NULLABLE.
"""
